...
MAX_ITERATIONS.times do |n|  
 ...
 new_paths = []
  ...
  #puts "#{n}: #{new_paths.length}" if n % 100 == 0

  break if(potential_paths.empty?)
end

if(longest_path.nil?)
  puts "unable to find path after #{MAX_ITERATIONS} iterations"
else
  puts "shortest path was #{longest_path} with a length of #{longest_path[:key].length - PASSCODE.length}"
end
